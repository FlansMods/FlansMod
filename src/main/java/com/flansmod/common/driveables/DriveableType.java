...
public abstract class DriveableType extends PaintableType
{
	...
	* -1 = It does not detect.<br> */
	...
	interface ParseFunc { void Parse(String[] split, DriveableType d); }
	private static HashMap<String, ParseFunc> parsers = new HashMap<String, ParseFunc>();
	static
	{
		// BASICS /////////////////////////////////////////////////////////////////////////////
		...
		{ 
			d.turretOrigin = new Vector3f(Float.parseFloat(split[1]) / 16F, Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F); 
		});
		...
		{ 
			d.wheelPositions[Integer.parseInt(split[1])] = new DriveablePosition(new Vector3f(Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F, Float.parseFloat(split[4]) / 16F), split.length > 5 ? EnumDriveablePart.getPart(split[5]) : EnumDriveablePart.coreWheel);
		});
		...
		{ 
			if(split.length > 4)
				d.seats[0] = new Seat(Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3]), Float.parseFloat(split[4]), Float.parseFloat(split[5]), Float.parseFloat(split[6]), Float.parseFloat(split[7]));
			else
				d.seats[0] = new Seat(Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3]));
		});
		...
		{ 
			...
			d.seats[Integer.parseInt(split[1])].gunOrigin = new Vector3f(Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F, Float.parseFloat(split[4]) / 16F);
		});
		parsers.put("RotatedDriverOffset", (split, d) -> 
		{ 
			d.seats[0].rotatedOffset = new Vector3f(Integer.parseInt(split[1]) / 16F, Integer.parseInt(split[2]) / 16F, Integer.parseInt(split[3]) / 16F);
		});
		...
		{ 
			d.seats[Integer.parseInt(split[1])].rotatedOffset = new Vector3f(Integer.parseInt(split[2]) / 16F, Integer.parseInt(split[3]) / 16F, Integer.parseInt(split[4]) / 16F);
		});
		parsers.put("PassengerAimSpeed", (split, d) -> 
		{ 
			d.seats[Integer.parseInt(split[1])].aimingSpeed = new Vector3f(Float.parseFloat(split[2]), Float.parseFloat(split[3]), Float.parseFloat(split[4]));
		});
		...
		{ 
			d.seats[Integer.parseInt(split[1])].pitchSoundLength = Integer.parseInt(split[2]);
		});
		...
		{ 
			d.turretOriginOffset = new Vector3f(Float.parseFloat(split[1]) / 16F, Float.parseFloat(split[2]) / 16F, Float.parseFloat(split[3]) / 16F);
		});
		...
		parsers.put("LeftLinkPoint", (split, d) -> 
		{ 
			d.leftTrackPoints.add(new Vector3f(split[1], d.shortName));
		});
		parsers.put("RightLinkPoint", (split, d) -> 
		{ 
			d.rightTrackPoints.add(new Vector3f(split[1], d.shortName));
		});
