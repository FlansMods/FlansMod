...
public class EntityVehicle extends EntityDriveable implements IExplodeable {
	...
	/**
	 * Wheel rotation angle. Only applies to vehicles that set a rotating wheels
	 * flag
	 */
	...
	/**
	 * Called with the movement of the mouse. Used in controlling vehicles if
	 * need be.
	 * 
	 * @param deltaY
	 * @param deltaX
	...
	public boolean pressKey(int key, EntityPlayer player) {
		...
		case 1: // Decelerate : Decrease the throttle, down to -1, or 0 if the
	...
	public void onUpdate() {
		...
		// Player is not driving this. Update its position from server update
		// packets
		if (worldObj.isRemote && !thePlayerIsDrivingThis) {
			// The driveable is currently moving towards its server position.
			// Continue doing so.
			...
			// If the driveable is at its server position and does not have the
			// next update, it should just simulate itself as a server side
			// driveable would, so continue
		}
		...
		for(EntityWheel wheel : wheels)
		{
			...
			if(canThrustCreatively || data.fuelInTank > data.engine.fuelConsumption * throttle)
			{
				...
				else
				{
					...
					/**if(adjYaw - (adjYaw*grip) > 5){
						//worldObj.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, wheel.posX, wheel.posY, wheel.posZ, 0,0,0);
			...
			} else if((type.floatOnWater && worldObj.isAnyLiquid(wheel.getBoundingBox())) && worldObj.isAnyLiquid(wheel.getBoundingBox().offset(0, 1 - 0, 0))){
			...
			//This part kills the throttle whenever we collide with something we can't step over.
			if((throttle >= 0.3 || throttle <= -0.3)){
				...
				boolean test1 = worldObj.isAirBlock(new BlockPos(Math.round(test1Pos.x), Math.round(test1Pos.y), Math.round(test1Pos.z)));
				boolean test2 = worldObj.isAirBlock(new BlockPos(Math.round(test1Pos.x), Math.round(test1Pos.y + type.wheelStepHeight), Math.round(test1Pos.z)));
				if(!test1 && !test2){
					throttle *= 0.6;
	...
	private Vec3 crossProduct(Vec3 a, Vec3 b) {
		return new Vec3(a.yCoord * b.zCoord - a.zCoord * b.yCoord, a.zCoord * b.xCoord - a.xCoord * b.zCoord,
